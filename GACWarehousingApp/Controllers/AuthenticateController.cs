...
namespace GACWarehousingApp.Controllers
{
    ...
    public class AuthenticateController : ControllerBase
    {
        ...
        private async Task<LoginModel> AuthenticateUser(LoginModel login)
        {
            ...
            if (login.UserName == "Abhishek" && login.Password=="123456")
            {
                user = new LoginModel { UserName = "Abhishek", Password = "123456" };
        ...
        public async Task<IActionResult> Login([FromBody] LoginModel data)
        {
            ...
            else
            {
                {
                    var tokenString = GenerateJSONWebToken(user);
                    response = NotFound(new { Token = tokenString, Message = "User Not Validated" });
                }
            }
            return response;
        }
